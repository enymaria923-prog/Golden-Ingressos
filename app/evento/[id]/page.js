...
export default function EventoPage() {
  ...
  const definirTipoIngressoAssento = (assentoKey, tipoIngressoId) => {
    setTiposIngressoPorAssento(prev => ({
      ...prev,
      [assentoKey]: tipoIngressoId
    }));
  };

  const calcularTotalLugarMarcado = () => {
    const ingressosDaSessao = ingressosPorSessao[sessaoSelecionada] || [];
    return assentosSelecionados.reduce((total, assento) => {
      const key = `${assento.setor}-${assento.fileira}-${assento.numero}`;
  ...
  const todosAssentosPossuemTipo = () => {
    if (assentosSelecionados.length === 0) return false;
          ...
          <div>
            <div style={{ marginBottom: '30px', textAlign: 'center' }}>
              ...
              <p style={{ color: '#666', fontSize: '14px', marginBottom: '15px' }}>
                Selecione o tipo de ingresso para cada assento
              </p>
              <button
                ...
                return (
                  <div key={key} style={{
                    ...
                    <h4 style={{ margin: '0 0 15px 0', fontSize: '20px', color: '#2c3e50' }}>
                      ...
                      {tipoSelecionado && <span style={{ color: '#27ae60', fontSize: '16px', marginLeft: '10px' }}>✅</span>}
                    </h4>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '15px' }}>
                      {ingressosDoSetor.map(ingresso => {
                        ...
                        return (
                          <button
                            key={ingresso.id}
                            onClick={() => definirTipoIngressoAssento(key, ingresso.id)}
                            style={{
                              padding: '20px',
            ...
            {!todosAssentosPossuemTipo() && assentosSelecionados.length > 0 && (
              <div style={{
                marginTop: '30px',
                padding: '20px',
                backgroundColor: '#fff3cd',
                ...
                <p style={{ margin: 0, color: '#856404', fontSize: '16px', fontWeight: 'bold' }}>
                  ⚠️ Selecione o tipo de ingresso para todos os assentos antes de continuar
                </p>
              </div>
            )}
          </div>
