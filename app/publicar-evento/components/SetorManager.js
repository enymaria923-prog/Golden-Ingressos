...
const SetorManager = ({ onSetoresChange }) => {
  ...
  const atualizarSetor = (setorId, campo, valor) => {
    setSetores(setores.map(setor => {
      if (setor.id === setorId) {
        ...
        // Se ativou lotes, cria um lote padrão
        if (campo === 'usaLotes' && valor === true && setor.lotes.length === 0) {
          ...
          novoSetor.tiposIngresso = []; // Limpa ingressos diretos
        }
        
        // Se desativou lotes, volta para ingressos diretos
  ...
  // ========== FUNÇÕES DE INGRESSO (DIRETO OU EM LOTE) ==========
  const adicionarTipoIngresso = (setorId, loteId = null) => {
    setSetores(setores.map(setor => {
      if (setor.id === setorId) {
        if (loteId) {
          // Adiciona ingresso dentro de um lote
          return {
            ...setor,
            lotes: setor.lotes.map(lote => {
              if (lote.id === loteId) {
          ...
          // Adiciona ingresso direto no setor
          return {
            ...setor,
            tiposIngresso: [
              ...setor.tiposIngresso,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ background: '#e8f4f8', padding: '15px', borderRadius: '8px', border: '2px solid #3498db' }}>
        ...
        <ul style={{ margin: 0, paddingLeft: '20px', fontSize: '14px', color: '#555' }}>
          <li><strong>Capacidade do Setor</strong>: Define quantos ingressos no total podem ser vendidos neste setor (opcional)</li>
          <li><strong>Lotes</strong>: Divida as vendas em fases (1º lote mais barato, 2º lote, etc.) com datas de início/fim</li>
          <li><strong>Tipos de Ingresso</strong>: Inteira, Meia, VIP, etc.</li>
          <li><strong>Regra</strong>: A soma dos ingressos não pode ultrapassar a capacidade do setor/lote</li>
        </ul>
      </div>
          ...
          <div key={setor.id} style={{ 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <div>
                ...
                <input type="number" value={setor.capacidadeTotal} onChange={(e) => atualizarSetor(setor.id, 'capacidadeTotal', e.target.value)} placeholder="Ex: 500" style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '5px', boxSizing: 'border-box', fontSize: '14px' }} />
                <small style={{ color: '#666' }}>Deixe em branco para capacidade ilimitada</small>
                      ...
                      <div key={lote.id} style={{ background: '#fff', padding: '15px', borderRadius: '8px', marginBottom: '15px', border: '2px solid #d1a7f5' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                          <div>
                            ...
                            <input type="text" value={lote.nome} onChange={(e) => atualizarLote(setor.id, lote.id, 'nome', e.target.value)} placeholder="Ex: 1º Lote" style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', boxSizing: 'border-box' }} />
                          </div>
                          <div>
                          ...
                          <div>
                            <label style={{ fontSize: '12px', display: 'block', marginBottom: '3px' }}>Fim (opcional)</label>
